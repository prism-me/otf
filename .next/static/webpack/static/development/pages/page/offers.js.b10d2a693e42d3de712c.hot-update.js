webpackHotUpdate("static\\development\\pages\\page\\offers.js",{

/***/ "./pages/page/offers/plans.js":
/*!************************************!*\
  !*** ./pages/page/offers/plans.js ***!
  \************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! react */ "./node_modules/react/index.js");
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var reactstrap__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! reactstrap */ "./node_modules/reactstrap/es/index.js");
var _this = undefined,
    _jsxFileName = "I:\\Prism Projects\\OTF\\otf\\pages\\page\\offers\\plans.js";

var __jsx = react__WEBPACK_IMPORTED_MODULE_0___default.a.createElement;


var plansData = [{
  icon: "/assets/images/OTF/icons/Premier.png",
  iconW: "/assets/images/OTF/icons/PremierW.png",
  title: "March 1 Offer",
  feature1: "Buy 2 Months Membership and get the 3rd",
  feature2: 'Month for 10dhs only.'
}, {
  icon: "/assets/images/OTF/icons/Premier.png",
  iconW: "/assets/images/OTF/icons/eliteW.png",
  title: "March 2 Offer",
  feature1: "8 classes/month",
  feature2: 'Extra classes at reduced rate'
}];

var Plans = function Plans() {
  return __jsx("section", {
    className: "plans",
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 25,
      columnNumber: 9
    }
  }, __jsx(reactstrap__WEBPACK_IMPORTED_MODULE_1__["Container"], {
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 26,
      columnNumber: 13
    }
  }, __jsx(reactstrap__WEBPACK_IMPORTED_MODULE_1__["Row"], {
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 27,
      columnNumber: 17
    }
  }, __jsx(reactstrap__WEBPACK_IMPORTED_MODULE_1__["Col"], {
    xs: "12",
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 28,
      columnNumber: 21
    }
  }, __jsx("div", {
    className: "title title2 title-inner",
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 29,
      columnNumber: 25
    }
  }, __jsx("div", {
    className: "main-title",
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 30,
      columnNumber: 29
    }
  }, __jsx("h2", {
    className: "borders text-center m-b-0 oftHeading",
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 31,
      columnNumber: 33
    }
  }, __jsx("span", {
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 32,
      columnNumber: 37
    }
  }, "plans to meet any need")))), __jsx("p", {
    className: "oftsubHeading",
    __self: _this,
    __source: {
      fileName: _jsxFileName,
      lineNumber: 36,
      columnNumber: 25
    }
  }, "Ready to change the way you look, the way you feel, and your life? From free trial workout to monthly packages, Orangetheory has a plan perfect for you. By investing in your health today, you\u2019re also investing in a longer, stronger and more vibrant future.")), plansData.map(function (item, i) {
    return __jsx(reactstrap__WEBPACK_IMPORTED_MODULE_1__["Col"], {
      sm: 4,
      key: i,
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 42,
        columnNumber: 29
      }
    }, __jsx("div", {
      className: "plans-container hover-overlay shadows bg-white text-center",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 43,
        columnNumber: 33
      }
    }, __jsx("div", {
      className: "plans-feature-container",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 44,
        columnNumber: 37
      }
    }, __jsx("div", {
      className: "feature-text",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 45,
        columnNumber: 41
      }
    }, __jsx("center", {
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 46,
        columnNumber: 45
      }
    }, __jsx("img", {
      src: item.icon,
      className: "feature-icon1 mb-4",
      alt: "icon",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 47,
        columnNumber: 49
      }
    }), __jsx("img", {
      src: item.iconW,
      className: "feature-icon2 d-none mb-4",
      alt: "icon",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 48,
        columnNumber: 49
      }
    })), __jsx("h4", {
      className: "plans-text-heading text-center bold text-uppercase font-primary",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 50,
        columnNumber: 45
      }
    }, item.title), __jsx("hr", {
      className: "set-border",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 51,
        columnNumber: 45
      }
    })), __jsx("div", {
      className: "plans-features font-primary",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 53,
        columnNumber: 41
      }
    }, __jsx("h5", {
      className: "plans-feature text-center mb-1",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 54,
        columnNumber: 45
      }
    }, item.feature1), __jsx("h5", {
      className: "plans-feature text-center",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 55,
        columnNumber: 45
      }
    }, item.feature2)), __jsx("a", {
      className: "otfBtn2",
      href: "#",
      __self: _this,
      __source: {
        fileName: _jsxFileName,
        lineNumber: 57,
        columnNumber: 41
      }
    }, "Book offer"))));
  }))));
};

/* harmony default export */ __webpack_exports__["default"] = (Plans);

/***/ })

})
//# sourceMappingURL=offers.js.b10d2a693e42d3de712c.hot-update.js.map